...
if __name__ == "__main__":
    ...
    cluster = LocalCluster(n_workers=2, threads_per_worker=20, memory_limit=memory_limit_per_worker) #20,2 ==> 2, 20 good!
    ...
    with dask.config.set(**{'array.slicing.split_large_chunks': False}):  # to allow large chunks & silent warning
        sum_array = sum_array.rechunk((100_000, 5, 5, 5)) #(100000, 20, 20, 20)
    ...
    for i, s in enumerate(batches_slice):
        ...
        print(f"Estimated time for one set of blocks {(time.time() - time_previous)/60} min")
