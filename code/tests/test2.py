...
if __name__ == "__main__":
    ...
    cluster = LocalCluster(n_workers=8, threads_per_worker=1, memory_limit=memory_limit_per_worker)
    ...
    with dask.config.set(**{'array.slicing.split_large_chunks': False}):  # to allow large chunks & silent warning
        sum_array = sum_array.rechunk((800, 10, 10, 10)) #(100000, 20, 20, 20)
    ...
    for i, s in enumerate(batches_slice):
        with ProgressBar():
            progress(dask.compute(all_blocks[s]))
